...
from action_photo_passion.services.images.service_image_details_reader import (
    ServiceImageReader,
)
...
class ImageState(rx.State):
    ...
    def get_image(self):
        """Retrieve the full details of an image."""
        ...
        self.img_gps_coordinates["gps-latitude"] = (
            image.get_exif_gps_latitude()
        )
        self.img_gps_coordinates["gps-altitude"] = (
            image.get_exif_gps_altitude()
        )
        self.img_gps_coordinates["gps-longitude"] = (
            image.get_exif_gps_longitude()
        )

        self.img_exif_standard_exposure_triangle["fstop"] = (
            image.get_exif_fstop()
        )
        self.img_exif_standard_exposure_triangle["iso"] = image.get_exif_iso()
        self.img_exif_standard_exposure_triangle["exposure-time"] = (
            image.get_exif_exposure_time()
        )
        ...
        self.camera_lens_data["camera-manufacturer"] = (
            image.get_exif_camera_manufacturer()
        )
        self.camera_lens_data["camera-model"] = image.get_exif_camera_model()
        self.camera_lens_data["lens-manufacturer"] = (
            image.get_exif_lens_manufacturer()
        )
