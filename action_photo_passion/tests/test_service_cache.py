...
class TestCacheService(unittest.TestCase):
    ...
    def test_image_not_existing_in_cache(self):
        self.assertFalse(cache.exists_image_in_cache(image_reader=self.image_parser),"The image should not be in the cache for the moment !!!")

    def test_image_added_to_cache(self):
        cache.add_image_to_cache(self.image_parser)
        self.assertTrue(cache.exists_image_in_cache(image_reader=self.image_parser),"The image should exist in the cache now !!!")

    def test_id_image_already_existing_in_cache(self):
        cache.add_image_to_cache(self.image_parser)
        self.assertTrue(cache.exists_image_id_in_cache(image_id=2)["status"],"The image should exist in the cache now !!!")
